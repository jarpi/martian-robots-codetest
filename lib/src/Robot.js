class Robot {
  ...
  run(instructions) {
    ...
    let [x, y, orientation] = robotInstructions[0].split(' ')
    this.position = {x: parseInt(x), y:parseInt(y)}
    console.dir(this.position)
    this.orientation = orientation
    let robotDirections = robotInstructions[1] ? robotInstructions[1].split('') : []
    console.dir(robotDirections)
    robotDirections.forEach(direction => console.log(direction))
  }
